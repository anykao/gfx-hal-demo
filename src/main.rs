...
fn main() {
    ...
    // Image
    let img_data = include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/logo.png"));
...
fn swapchain_stuff(
    ...
    let pipeline = {
        let vs_module = {
            let glsl = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/quad.vert"));
        ...
        let fs_module = {
            let glsl = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/assets/quad.frag"));
